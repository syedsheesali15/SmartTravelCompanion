...
extension TravelPlaceSubtitle on PlaceEntity {
  ...
  String get subtitleLine {
    final parts =
        locationLine.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toList();
...
class TravelPlaceTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final subtitleColor =
        light ? const Color(0xFF64748B) : Theme.of(context).colorScheme.onSurfaceVariant;
    final heartMuted = light ? const Color(0xFFCBD5E1) : Colors.white54;

    final shape = RoundedRectangleBorder(
      borderRadius: BorderRadius.circular(_radius),
      side: BorderSide(color: light ? const Color(0xFFF1F5F9) : Theme.of(context).colorScheme.outlineVariant.withValues(alpha: .35)),
    );
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        ...
                        icon: Icon(
                          place.isFavorite ? Icons.favorite_rounded : Icons.favorite_outline,
                          color: place.isFavorite ? AppColors.accentHeart : heartMuted,
                          size: 24,
                        ),
                      ),
                    ],
